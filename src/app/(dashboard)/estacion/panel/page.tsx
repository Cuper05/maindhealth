...
/** Herramientas de personal (citas, intake). La Dell usa /estacion (standby). */
export default async function EstacionPanelPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Herramientas de personal. En la Dell deja abierta /estacion (pantalla corporativa); el video se abre solo."
        action={
      ...
      {waitingDoctor.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-semibold text-slate-900">
            Esperando teleconsulta ({waitingDoctor.length})
          </h2>
          <div className="space-y-3">
            {waitingDoctor.map((item) => (
              <article
                key={`${item.appointmentId}-${item.sessionId}`}
