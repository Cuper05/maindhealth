...
try {
  ...
  console.log("navigate sala/11");
  try {
    await b.send(
      "Page.navigate",
      { url: "https://health.maindsteel.com.mx/estacion/sala/11" },
      sessionId,
      5000,
    );
    console.log("navigated");
  } catch (err) {
    console.log("nav", err instanceof Error ? err.message : err);
  }
  ...
  console.log(JSON.stringify(locked.result?.result?.value || locked, null, 2));
} finally {
  b.close();
}
